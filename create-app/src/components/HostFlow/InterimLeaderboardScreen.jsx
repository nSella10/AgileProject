...
const InterimLeaderboardScreen = ({
  ...
  useEffect(() => {
    ...
    // ניקוי כשיוצאים מהקומפוננטה - לא עוצרים את השמע כדי לשמור על המשכיות
    return () => {
      console.log(
        "🔄 Leaderboard cleanup - keeping audio playing for seamless transition"
      );
      // לא עוצרים את השמע המקומי אם הוא זהה לשמע המשותף
      if (audioRef.current && audioRef.current !== sharedAudioRef) {
